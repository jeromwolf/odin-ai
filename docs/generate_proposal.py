...
def add_screenshot(filename, caption, width=Inches(5.5)):
    ...
    try:
        ...
        p.alignment = WD_ALIGN_PARAGRAPH.CENTER
        run = p.add_run()
        run.add_picture(insert_path, width=width)
...
# 4.4 알림 시스템
add_page_break()
doc.add_heading('4.4 맞춤형 알림 시스템', level=2)
...
# 4.5 지식 그래프
add_page_break()
doc.add_heading('4.5 AI 지식 그래프 탐색기', level=2)
