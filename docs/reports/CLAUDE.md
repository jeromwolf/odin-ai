...
## Project Context Summary (2025-09-18)
...
#### **2025-09-18 추가 완료 작업**
- ✅ **데이터베이스 스키마 최적화 완료**: 검색 성능 문제 해결
- ✅ **하이브리드 저장 전략 구현**: 마크다운 파일 + DB 메타데이터
- ✅ **3단계 검색 시스템 구축**: 인덱스 → 전체 텍스트 → 파일 검색
- ✅ **PostgreSQL 전체 텍스트 검색**: tsvector + GIN 인덱스 구현
- ✅ **19개 검색 인덱스 생성**: B-tree + GIN 복합 인덱싱
- ✅ **ntceSpecDocUrl1~10 모든 첨부파일 지원**: 완전한 문서 수집
...
- **고도화 마크다운**: 7,040자 구조화된 문서 생성 (이모지 포함) ✅
- **한글 인코딩 완벽 해결**: pyhwp 패키지 + 올바른 CLI 호출로 100% 한글 처리 ✅
...
#### **데이터베이스 스키마 설계 (2025-09-18 완료)**
```sql
-- 7개 테이블 구조
├── bid_announcements      # 입찰 공고 메인 (22개 필드)
├── bid_documents          # 첨부 문서 (16개 필드, 최적화)
├── bid_document_search    # 검색 전용 (16개 필드, 19개 인덱스!)
├── collection_logs        # 수집 로그
├── search_keywords        # 검색 통계
├── users                  # 사용자
└── user_bid_bookmarks     # 북마크
...
#### **실제 처리 결과 예시 (2025-09-17 검증)**
```
📥 다운로드: 수의계약안내공고[어린이보호구역(신안초등학교) 보행로 조성사업].hwp (128,512 bytes)
🔧 처리방법: HWP Viewer CLI (pyhwp 기반)
📄 텍스트 추출: 완벽한 한글 처리 ✅
🎨 마크다운: 7,040자, 171줄 (이모지 포함 구조화 문서)
📅 주요 일정: 2025.9.18(목) 10:00, 2025.9.22(월) 10:00 자동 인식
🏢 관련 기관: 산청군 신안면, 조달청 자동 추출
💰 관련 금액: 예정가격, 기초금액 정보 자동 강조
⚠️ 특이사항: 노무비 미지급 통보, 하도급지킴이 의무사항 자동 하이라이팅
```

### 핵심 기술 결정사항 (업데이트)
1. **데이터 수집**: 공공데이터포털 API (90%) + Selenium 파일 다운로드 (10%)
2. **데이터 저장**: PostgreSQL (메타데이터) + 파일 시스템 (HWP/PDF 원본)
3. **HWP 처리**: pyhwp (기본) + tools/hwp-viewer (CLI) + 마크다운 구조화
...
1. ✅ 공공데이터포털 API 연동 (완료)
2. ✅ HWP 파일 처리 파이프라인 구현 (완료)
3. ✅ PostgreSQL 데이터베이스 설계 및 구현 (완료)
4. ✅ ntceSpecDocUrl1~10 기반 자동 다운로드 시스템 (완료)
5. ✅ 검색 및 필터링 시스템 (완료)
6. 🔄 이메일 알림 시스템 (다음 작업)
7. 🔄 사용자 인증 시스템 (다음 작업)
...
# Critical HWP dependencies (required for document processing)
pip install pyhwp==0.1b15    # Korean HWP file processing
pip install olefile==0.47    # OLE file format support
pip install cryptography==45.0.7  # Security for HWP parsing
...
### Docker Commands
```bash
# Build containers (ensure pyhwp is included in Dockerfile)
...
# Important: Docker environments must include HWP processing dependencies
# Add to Dockerfile: RUN pip install pyhwp==0.1b15 olefile==0.47
```
...
2. **Technical Challenges**
   - HWP file processing: pyhwp (primary) + tools/hwp-viewer CLI for structured markdown
   - Korean encoding: Ensure UTF-8 throughout pipeline (solved with pyhwp==0.1b15)
